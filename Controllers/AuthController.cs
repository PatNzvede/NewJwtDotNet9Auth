...
using NewJwtDotNet9Auth.Services;
...
namespace NewJwtDotNet9Auth.Controllers
{
    ...
    public class AuthController(IAuthService authService) : ControllerBase
    {
        ...
        public async Task<ActionResult<User>> Register(UserDto request)
        {
           var user = await authService.RegisterAsync(request);
            if (user is null)
                return BadRequest("Username lready exists");
            return Ok(user);
        }
        //}
        [HttpPost("login")]
        ...
        {
            return Ok("You should be admin to be are authenticated");
        }
